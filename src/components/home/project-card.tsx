...
export const ProjectCard = ({ project, index }: {
    ...
    return (
        <motion.div initial={{ opacity: 0, y: 40 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-100px" }} transition={{ duration: 0.8, delay: index * 0.1 }} className="group relative w-full">
            <Link href={`/projects/${project.slug}`} className="block w-full">

                <div className={cn("relative w-full aspect-[1] md:aspect-[16/9] rounded-[2.5rem] overflow-hidden transition-all duration-700 [mask-image:radial-gradient(white,black)]", "bg-white/40 backdrop-blur-xl", "group-hover:shadow-2xl group-hover:shadow-accent/10")}>
