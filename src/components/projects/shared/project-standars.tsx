...
export const ProjectStandards = ({ project }: {
    ...
    return (<section className="w-full border-t border-border/40 relative overflow-hidden py-24 md:py-48">
            ...
            return (<motion.div key={item.title} initial={{ opacity: 0, y: 40 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: idx * 0.1, duration: 0.5 }} className="group relative bg-white/60 backdrop-blur-md border border-white/60 p-6 md:p-8 rounded-[1.5rem] md:rounded-[2rem] shadow-sm hover:shadow-lg hover:-translate-y-1 transition-colors transition-transform transition-shadow duration-500 flex flex-col justify-between overflow-hidden min-h-[280px] md:min-h-[320px]">

                                <span className="absolute -right-4 -top-4 text-[6rem] md:text-[8rem] font-display font-bold text-foreground/[0.03] select-none leading-none group-hover:text-foreground/[0.05] transition-colors duration-500">
                                    0{idx + 1}
                                </span>

                                <div className="relative z-10">

                                    <div className="w-12 h-12 md:w-14 md:h-14 bg-white rounded-xl md:rounded-2xl border border-border flex items-center justify-center mb-6 md:mb-8 shadow-sm group-hover:scale-110 transition-transform duration-500">
