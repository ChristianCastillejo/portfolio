...
export const ProjectHero = ({ project }: ProjectHeroProps) => {
    ...
    return (<section ref={containerRef} className="relative w-full max-w-[1600px] mx-auto px-6 md:px-12 pt-44 md:pt-40 min-h-[90vh] flex flex-col justify-between">
            ...
            <motion.div style={{ y }} className="relative w-full aspect-[3/4] md:aspect-[16/9] lg:aspect-[21/9] mt-12 mb-16 md:mt-16 md:mb-24 bg-card rounded-[1.5rem] md:rounded-[2rem] overflow-hidden shadow-sm border border-border/50">
                <video ref={videoRef} src={project.video} muted loop playsInline className="w-full h-full object-cover opacity-95 mix-blend-multiply grayscale-[20%]"/>
            ...
            return (<div key={tech.name} className="flex items-center gap-2 group">
                    ...
                    <div className="md:col-span-4 md:px-8 flex flex-col gap-4">
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-2 gap-x-4 gap-y-6">
                            {project.metrics && project.metrics.length > 0 ? (project.metrics.map((metric, idx) => (<div key={idx} className="flex flex-col">
                                        <div className="flex items-baseline gap-2 text-foreground font-display text-3xl font-bold mb-1">
                                            {idx === 0 ? (<Zap size={20} className="fill-accent text-accent"/>) : (<BarChart3 size={20} className="text-accent"/>)}
                                            <span>{metric.value}</span>
                                        </div>
                                        <p className="text-xs text-foreground/60 font-medium leading-relaxed text-pretty">
                                            {metric.label}
                                        </p>
                                    </div>))) : (<div className="flex items-center gap-2 text-foreground/40">
                                    <BarChart3 size={20}/>
