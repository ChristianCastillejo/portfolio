...
export const ProjectComponentLabButton = ({ project }: {
    project: ProjectCaseStudy;
}) => {
    if (!project.componentLab)
        return null;
...
>
  Click Me
    ...
    return (<section className="w-full max-w-[1400px] mx-auto px-6 md:px-12 py-24 md:py-48 border-t border-border/40 relative overflow-hidden">
                ...
                <div className="bg-white/60 backdrop-blur-md border border-white/60 p-6 md:p-8 rounded-[1.5rem] md:rounded-[2rem] shadow-sm flex flex-col gap-6 md:gap-8">
                    ...
                    <div className="space-y-5">
                        ...
                        <div className="flex flex-wrap gap-3 !mt-1 md:!mt-2">
                            {["primary", "secondary", "ghost"].map((v) => (<button key={v} onClick={() => setVariant(v as any)} className={cn("px-4 py-2 text-sm font-medium rounded-full border transition-all duration-300 capitalize", variant === v
                    ...
                    <div className="space-y-5">
                        ...
                        <div className="flex flex-wrap gap-3 !mt-1 md:!mt-2">
                            {["sm", "default", "lg"].map((s) => (<button key={s} onClick={() => setSize(s as any)} className={cn("px-4 py-2 text-sm font-medium rounded-full border transition-all duration-300 capitalize", size === s
                                ? "bg-foreground text-background border-foreground shadow-md transform scale-105"
                                : "bg-transparent border-border text-foreground/60 hover:border-foreground/30 hover:bg-white/50")}>
                                {s}
                            </button>))}
                    ...
                    <div className="space-y-5">
                        ...
                        <div className="flex items-center justify-between p-1 !mt-1 md:!mt-2 bg-white/40 rounded-full border border-border/60 relative w-full max-w-[240px]">
                            ...
                            <button onClick={() => setIsLoading(false)} className={cn("w-1/2 py-2 text-sm font-bold rounded-full transition-all duration-300 z-10", !isLoading ? "bg-foreground shadow-sm text-background" : "text-foreground/50 hover:text-foreground/80")}>
                                Idle
                            </button>
                            <button onClick={() => setIsLoading(true)} className={cn("w-1/2 py-2 text-sm font-bold rounded-full transition-all duration-300 z-10 flex items-center justify-center gap-2", isLoading ? "bg-foreground shadow-sm text-background" : "text-foreground/50 hover:text-foreground/80")}>
                                <Sparkles size={12} />
                                Loading
                            </button>
                        </div>
            ...
            <div className="lg:col-span-8 flex flex-col gap-8 min-w-0">
                ...
                <div className="flex-1 min-h-[300px] md:min-h-[400px] bg-white border border-white/60 rounded-[1.5rem] md:rounded-[2.5rem] shadow-sm relative overflow-hidden flex items-center justify-center">
                    ...
                    <div className="relative z-10 transform scale-100 md:scale-[1.2]">
                        <div className="p-8">
                            <Button variant={variant as ButtonProps["variant"]} size={size} isLoading={isLoading} className="shadow-2xl shadow-accent/20">
                                Click Me
                            </Button>
                        </div>
                    </div>
                ...
                <motion.div layout className="w-full">
                    <CodeWindow title="Button.usage.tsx" lang="tsx" code={codeSnippet} className="shadow-xl" />
                </motion.div>
            </div>
