...
const StoryVisual = ({ step }: {
    step: StoryStep;
}) => {
    if (step.visualType === "code" && step.codeSnippet) {
        return (<div className="w-full">
            <CodeWindow code={step.codeSnippet} lang={step.codeLanguage || 'typescript'} title={step.title.toLowerCase().replace(/\s/g, '-') + '.' + (step.codeLanguage === 'bash' ? 'sh' : 'ts')} />
        </div>);
    }
    if (step.visualType === "image" && step.visualContent) {
        return (<div className="relative aspect-video rounded-[2rem] overflow-hidden bg-card">

            {typeof step.visualContent === 'string' ? (<Image src={step.visualContent} alt={step.title} fill className="object-cover" />) : null}
...
export const ProjectStory = ({ project }: {
    ...
    return (<section className="w-full max-w-[1600px] mx-auto px-6 md:px-12 py-32 md:py-48 bg-background overflow-hidden">
        ...
        <div className="max-w-4xl mx-auto text-center mb-24 md:mb-40">
            ...
            <motion.h2 initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: 0.1 }} className="text-foreground font-display text-4xl sm:text-5xl md:text-7xl font-bold tracking-tight leading-[0.9]">
                Engineering narrative.
            </motion.h2>
        </div>

        <div className="flex flex-col gap-32 md:gap-48">
            {project.storySteps.map((step, index) => (<motion.div key={step.id} initial={{ opacity: 0, y: 40 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-100px" }} transition={{ duration: 0.8, ease: [0.16, 1, 0.3, 1] }} className="relative">
                ...
                <div className="relative z-20 max-w-6xl mx-auto md:pl-0">
                    ...
                    {index !== project.storySteps!.length - 1 && (<div className="absolute left-[50%] -bottom-24 h-24 w-px bg-border/50 hidden md:block"></div>)}
