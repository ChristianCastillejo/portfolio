...
const buttonVariants = cva("group relative inline-flex items-center justify-center gap-3 px-8 py-3 rounded-full font-display font-medium text-sm border transition duration-300 ease-out cursor-pointer disabled:pointer-events-none disabled:opacity-50 w-full md:w-auto", {
    variants: {
        variant: {
...
const Button = React.forwardRef<HTMLButtonElement | HTMLAnchorElement, ButtonProps>(({ className, variant, stable, size, asChild = false, isLoading = false, icon: Icon, href, children, ...props }, ref) => {
    const internalContent = (<>
                <span className={cn(isLoading ? "opacity-0" : "opacity-100")}>{children}
                </span>
                ...
                {isLoading && (<span className="absolute inset-0 flex items-center justify-center">
                        <svg className="animate-spin h-5 w-5 text-current" viewBox="0 0 26.349 26.35" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
                            <g><g>
    ...
    return (<button className={commonClasses} disabled={props.disabled || isLoading} ref={ref as React.Ref<HTMLButtonElement>} {...props}>
                {internalContent}
            </button>);
});
